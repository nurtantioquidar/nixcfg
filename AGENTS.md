...
- Keep Herdr user-managed through the pinned upstream flake input and `nix/home/herdr.nix`; update it with `nix flake update herdr` and rebuild Home Manager. Its config is also owned by that module, with mouse capture disabled to prevent SGR mouse reports from leaking into the shell after detach.
